...
import { useGetMachineQuery, useUpdateMachineMutation } from "@/redux/machines/machinesApiSlice";
...
export default function MachineEditModal({ handleModalOpen, id }: { handleModalOpen: (isOpen: boolean) => void; id: string }) {
  const { data, isLoading, error, isError, refetch } = useGetMachineQuery(id);
...
const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    try {
      await updateMachine(formData).unwrap();
      ...
      if (fetchError.status === 409) {
        ...
        toast.error("Machine update failed");
      }
    }
  };
