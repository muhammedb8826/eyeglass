...
import { useGetNonStockServiceQuery, useUpdateNonStockServiceMutation } from "@/redux/services/nonStockServicesApiSlice";
...
export default function NonStockServicesEditModal({ handleModalOpen, id }: { handleModalOpen: (isOpen: boolean) => void; id: string }) {
const {data, isLoading, error, isError, refetch} = useGetNonStockServiceQuery(id);
const [updateNonStockService, { isLoading: isUpdating }] = useUpdateNonStockServiceMutation();
...
useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
     try {
       await updateNonStockService(formData).unwrap();
        toast.success("Non Stock Service updated successfully");
        refetch();
       ...
       const fetchError = error as FetchBaseQueryError;
        if (fetchError.status === 409) {
          ...
          toast.error("Non Stock Service update failed");
        }
     }
  };
