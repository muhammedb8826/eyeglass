...
export const PurchaseRegistration = () => {
  ...
  const handleItemChange = (index: number, value: string) => {
    ...
    if (selectedItem) {
      ...
      // Fetch base variants for this item so the Variant selector has options.
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    for (const row of formData) {
      ...
      // Ensure we know whether this item is variant-tracked before validating.
    ...
    const payloadItems: PurchaseItem[] = formData.map((item) => {
