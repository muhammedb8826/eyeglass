import { useCreateNonStockServiceMutation } from "@/redux/services/nonStockServicesApiSlice";
...
type NonStockServicesRegistrationProps = {
  handleModalOpen: (value: boolean) => void;
};

export const NonStockServicesRegistration = ({ handleModalOpen }: NonStockServicesRegistrationProps) => {
  const [createNonStockService, {isLoading}] = useCreateNonStockServiceMutation();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
   try {
    await createNonStockService(formData).unwrap();
    toast.success("Non Stock Service added successfully");
    ...
    if (fetchError.status === 409) {
      ...
      toast.error("Non Stock Service creation failed");
    }
   }
  };
