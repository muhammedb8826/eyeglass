...
export const Purschase = () => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          ...
          const errorMessage = handleApiError(fetchError, 'Failed to delete the purchase');
  ...
  const { purchases, total } = data;
  const totalPages = Math.ceil(total / limit);

  const purchaseListContent = purchases.map((purchase, index) => (
    <tr key={purchase.id}>
      <td className="border-b flex items-center border-[#eee] py-5 px-4 dark:border-strokedark">
        <NavLink to={`/dashboard/inventory/purchases/${purchase.id}`} className="flex items-center gap-4 text-graydark/80 hover:text-primary hover:underline lg:text-base">
          <BiPurchaseTag />
          {purchase.series}
        </NavLink>
      </td>
      ...
      <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
        {purchase.purchaser.first_name} {purchase.purchaser.last_name}
      </td>
      <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
        {purchase.amount}
      </td>
