...
interface StoreRequestNotificationTableProps {
  title: string;
  orders: SaleItem[];
...
export const StoreRequestNotificationTable = ({
  ...
  expandedNotes,
  setExpandedNotes,
}: StoreRequestNotificationTableProps) => {
  const [newNote, setNewNote] = useState<SaleItemNoteType>({
    saleItemId: '',
    text: '',
    userId: '',
  });
  ...
  const handleChangeNotes = (value: string, saleItemId: string, userId: string) => {
    setNewNote({ saleItemId, text: value, userId });
  };
                          ...
                          <td colSpan={8} className="p-2 border-b border-[#eee] dark:border-strokedark">
                            <div className="relative">
                              <textarea
                                ...
                                // value={newNote.text || (data.saleItemNotes && data.saleItemNotes[index]?.text) || ""}
                              ...
                              <button
                                ...
                                onClick={() =>
                                  handleUpdateNote(
                                    {
                                      saleItemId: data.id,
                                      text: newNote.text || (data.saleItemNotes && data.saleItemNotes[index]?.text) || "",
                                      userId: user?.id || "",
                            ...
                            {data.saleItemNotes && data.saleItemNotes.length > 0 && (
                              <div className="mt-4">
                                <h4 className="font-medium">Previous Notes:</h4>
                                <ul className="list-disc ml-4">
                                  {data.saleItemNotes.map((note, noteIndex) => (
