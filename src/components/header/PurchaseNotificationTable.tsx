...
interface PurchaseNotificationTableProps {
  title: string;
  orders: PurchaseItem[];
  handleAction: (index: number) => void;
  showPopover: null | number;
  handleModalOpen: (id: string, status: string, index: number) => void;
  handleUpdateNote: (
    newNote: PurchaseItemNoteType,
    index: number,
...
export const PurchaseNotificationTable = ({
  ...
  expandedNotes,
  setExpandedNotes,
}: PurchaseNotificationTableProps) => {

  const [newNote, setNewNote] = useState<PurchaseItemNoteType>({
    purchaseItemId: '',
    text: '',
    userId: '',
  });
  ...
  const handleChangeNotes = (value: string, purchaseItemId: string, userId: string) => {
    setNewNote({ purchaseItemId, text: value, userId });
  };
                          ...
                          {showPopover === index && (
                            ...
                            >
                              <ul className="py-2 text-sm text-gray-700">
                                ...
                                {status2.value && data.status !=="Returned" && (
                                  <li>
                                    <button
                                      type="button"
                                      onClick={() =>
                          ...
                          <td colSpan={8} className="p-2 border-b border-[#eee] dark:border-strokedark">
                            <div className="relative">
                              <textarea
                                ...
                                // value={newNote.text || (data.purchaseItemNotes && data.purchaseItemNotes[index]?.text) || ""}
                                  ...
                                  handleUpdateNote(
                                    {
                                      ...
                                      text: newNote.text || (data.purchaseItemNotes && data.purchaseItemNotes[index]?.text) || "",
                                      userId: user?.id || "",
                                    }, // Pass a single note object instead of an array
                                    index,
                                    setExpandedNotes,
                                    expandedNotes
                                  )
                            ...
                            {data.purchaseItemNotes && data.purchaseItemNotes.length > 0 && (
                              <div className="mt-4">
                                <h4 className="font-medium">Previous Notes:</h4>
                                <ul className="list-disc ml-4">
                                  {data.purchaseItemNotes.map((note, noteIndex) => (
