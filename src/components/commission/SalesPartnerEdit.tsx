import { useGetSalesPartnerQuery, useUpdateSalesPartnerMutation } from "@/redux/sales-partner/salesPartnersApiSlice";
...
const SalesPartnerEdit = ({ handleModalOpen, id }: { handleModalOpen: (isOpen: boolean) => void; id: string }) => {
 
    const {data, isLoading, error, isError, refetch} = useGetSalesPartnerQuery(id)
    const [updateSalesPartner, {isLoading: isUpdating}] = useUpdateSalesPartnerMutation()
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        try {
            await updateSalesPartner(formData).unwrap();
            ...
            if (fetchError.status === 409) {
                ...
                toast.error('Partner update failed');
            }
        }
    }
...
export default SalesPartnerEdit
