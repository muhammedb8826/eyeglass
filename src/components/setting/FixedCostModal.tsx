...
import { useCreateFixedCostMutation } from "@/redux/fixed-cost/fixedCostApiSlice";
...
interface BackendErrorResponse {
  ...
  type FixedCostModalProps = {
    handleModalOpen: (value: boolean) => void;
  };

const FixedCostModal = ({ handleModalOpen }: FixedCostModalProps) => {
    const user = useSelector(selectCurrentUser);
    const [createFixedCost, { isLoading: isCreating }] = useCreateFixedCostMutation();
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if(user?.roles === 'ADMIN'){
            ...
            try {
                await createFixedCost(data).unwrap();
                ...
                if (fetchError.status === 409) {
                    ...
                    const errorMessage = errorData?.message?.message || 'A fixed cost with this description already exists.';
                    toast.error(errorMessage);
                    handleModalOpen(false);
                } else {
                    toast.error('Failed to create fixed cost');
                }
            }
        }
        else {
            toast.error('You are not authorized to create fixed cost');
            handleModalOpen(false);
        }
    }
    if(isCreating){
