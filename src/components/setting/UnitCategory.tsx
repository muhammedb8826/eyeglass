...
import { useDeleteUnitMutation, useGetUnitsQuery } from "@/redux/unit/unitApiSlice";
...
import UnitCategoryRegistration from "./UnitCategoryRegistration";
import UnitCategoryEditModal from "./UnitCategoryEditModal";
...
const UnitCategory = () => {
  ...
  const { data, isLoading, isError, error } = useGetUnitsQuery({ page, limit });
  ...
  const { unitCategories, total } = data;
  const totalPages = Math.ceil(total / limit);


  const unitsContent = unitCategories?.map((unit) => (
    <tr key={unit.id}>
      <td className="border-b border-[#eee] py-5 px-4 pl-9 dark:border-strokedark xl:pl-11">
        <NavLink to={`/dashboard/settings/unit-category/${unit.id}`} className="hover:text-primary">
          <h5 className="font-medium text-black dark:text-white hover:text-primary hover:underline">
            {unit.name}
          </h5>
        </NavLink>
      </td>
      <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
        <p className="text-black dark:text-white">{unit.description}</p>
      </td>
      <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
        ...
        {activeTabId === 'units' ?
          (isLoading ? (
            ...
            <>
              ...
              {isModalOpen && (<UnitCategoryRegistration handleModalOpen={handleModalOpen} />)}
              {updateModalOpen && id && (
                <UnitCategoryEditModal handleModalOpen={handleModalClose} id={id} />
              )}
            </>
          )) : null

        }
        {activeTabId === 'unitAttributes' && <UnitAttributes />}
      </div>
    </>
  );
};
