...
const FilePathEditModal = ({
  ...
  const { data, isLoading, error, isError, refetch } = useGetFilePathQuery(id);
  const [updateFilePath, { isLoading: isUpdating }] =
    useUpdateFilePathMutation();
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await updateFilePath(data).unwrap();
      ...
      if (fetchError.status === 409) {
        ...
        toast.error("File path update failed");
      }
    }
  };
...
export default FilePathEditModal;
