...
import { useCreateUnitAttributeMutation } from "@/redux/unit/unitAttributeSlice";
...
type UnitAttributeRegistrationProps = {
    handleModalOpen: (value: boolean) => void;
}; 

const UnitAttributeRegistration = ({ handleModalOpen }: UnitAttributeRegistrationProps) => {
    const [createUnitAttribute, { isLoading }] = useCreateUnitAttributeMutation();
...
const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        await createUnitAttribute(data).unwrap();
        toast.success('Unit attribute added successfully')
        ...
        const fetchError = error as FetchBaseQueryError;
            if (fetchError.status === 409) {
                ...
                toast.error('Unit attribute add failed')
            }
    }
}
      ...
      <div className="relative w-auto my-6 mx-auto max-w-3xl ">
        ...
        <div className="border-0 rounded-lg relative flex flex-col w-full bg-white shadow-default dark:border-strokedark dark:bg-boxdark outline-none focus:outline-none">
          ...
          <div className="flex items-start justify-between border-b border-stroke py-4 px-6.5 dark:border-strokedark rounded-t">
            <h3 className="text-3xl text-black dark:text-white font-semibold text">Add Unit Attribute</h3>
            <button
              title="close"
              type="button"
...
export default UnitAttributeRegistration
