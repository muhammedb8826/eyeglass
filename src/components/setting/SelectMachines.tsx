...
const SelectMachines: React.FC<DropdownProps> = ({id, selectedIds, setSelectedIds}) => {

const {data: machines, isLoading} = useGetAllMachinesQuery()
    ...
    useEffect(() => {
      if (machines) {
        const newOptions: Option[] = machines.map((machine) => ({
          value: machine.id,
          text: machine.name,
          selected: false,
        }));
   ...
   const select = (index: number, event: React.MouseEvent) => {
    ...
    return (
      <div className="relative z-50">
        ...
        <div>
          <select title="select machines" className="hidden" id={id}>
