...
import FixedCostEditModal from "./FixedCostEditModal";

const FixedCost = () => {
  const user = useSelector(selectCurrentUser);
  const [page, setPage] = useState(1);
  const [limit] = useState(10);
  const { data, isLoading, isError, error } = useGetFixedCostsQuery({
    page,
    limit,
  });
  const [deleteFixedCost, { isLoading: isDeleting }] =
    useDeleteFixedCostMutation();
  ...
  const handleDeleteFixedCost = (id: string) => {
    if (user?.roles === "ADMIN") {
      Swal.fire({
        title: "Are you sure?",
        text: "You want to delete this fixed cost!",
        icon: "warning",
        ...
        if (result.isConfirmed) {
          try {
            await deleteFixedCost(id).unwrap();
  ...
  const { fixedCosts, total } = data;
  const totalPages = Math.ceil(total / limit);

  const fixedCostsContent = fixedCosts?.map((fixedCost) => (
    <tr key={fixedCost.id}>
      ...
      <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
        <p className="text-black dark:text-white">{fixedCost.dailyFixedCost}</p>
      </td>
      <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
        <div className="flex items-center space-x-3.5">
          <button
            onClick={() => handleDeleteFixedCost(fixedCost.id || "")}
            disabled={isDeleting}
          ...
          <button
            onClick={() => handleUpdateModalOpen(fixedCost.id || "")}
            type="button"
            title="edit"
  ...
  return (
    <>
      <Breadcrumb pageName="Fixed Cost" />
      ...
      <div className="flex items-center justify-between flex-column flex-wrap md:flex-row space-y-4 md:space-y-0 pb-4">
        {user?.roles === "ADMIN" && (
          <div>
            ...
            >
              <MdCategory />
              <span className="ml-2">Add new fixed cost</span>
            </button>
          </div>
        )}
        ...
        <div className="relative">
          ...
          <input
            ...
            placeholder="Search for fixed cost"
          />
        </div>
      </div>
      ...
      {isModalOpen && <FixedCostModal handleModalOpen={handleModalOpen} />}
      {updateModalOpen && id && (
        <FixedCostEditModal handleModalOpen={handleModalClose} id={id} />
      )}
    </>
  );
};
