...
export const OrderRegistration = () => {
  ...
  const fetchOrderInfoForRow = (
    ...
    fetch(url, { headers })
      ...
      .then((response: ItemOrderInfoResponse & { data?: ItemOrderInfoResponse }) => {
        ...
        if (!pricing) {
          const basesOnItem = item?.itemBases;
          const hasBases =
            Array.isArray(basesOnItem) && basesOnItem.length > 0;
          if (hasBases && !itemBaseId) {
            return;
          }
  ...
  const handleItemChange = (index: number, value: string) => {
    ...
    if (selectedItem) {
      ...
      if (selectedItem.id) {
        if (selectedItem.id in itemBasesMap) {
          const bases = itemBasesMap[selectedItem.id];
